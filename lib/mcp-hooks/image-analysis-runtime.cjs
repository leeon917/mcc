...
const log = require('../shared/logger.cjs')('ImageRuntime');
...
function analyzeViaCliProxy(base64Data, mediaType, model, prompt, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    log.debug(`API call endpoint=${endpoint.toString()} model=${model} format=${format} prompt=${prompt.substring(0, 100)} imageSize=${Math.round(base64Data.length * 3 / 4 / 1024)}KB`);
    ...
    const req = transport.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          log.debug(`Response status=${res.statusCode} bodyLength=${data.length}`);
          if (res.statusCode >= 400) {
            log.debug(`Error response: ${data.substring(0, 300)}`);
          }
...
async function analyzeWithRetry(base64Data, mediaType, prompt, timeoutMs) {
  ...
  for (const [index, model] of models.entries()) {
    try {
      log.debug(`Trying model ${index + 1}/${models.length} model=${model}`);
...
async function analyzeFile(filePath, options = {}) {
  ...
  log.debug(`Starting image analysis file=${path.basename(filePath)} size=${(stats.size / 1024).toFixed(1)}KB provider=${getCurrentProvider() || 'unknown'} model=${model} models=${getModelsToTry().join('->')} timeout=${timeoutMs / 1000}s endpoint=${getRuntimeEndpoint()} template=${template} promptSource=${promptSource}`);

  const base64Data = encodeFileToBase64(filePath);
  const mediaType = getMediaType(filePath);
  log.debug(`File encoded mediaType=${mediaType} base64Length=${(base64Data.length / 1024).toFixed(1)}KB`);

  const result = await analyzeWithRetry(base64Data, mediaType, prompt, timeoutMs);
  log.debug(`Analysis complete responseLength=${result.description.length}chars model=${result.model} template=${template}`);
