...
function debugLog(message, data = {}) {
  if (!process.env.MCC_DEBUG) return;
...
const DEBUG_RUNTIME = process.env.MCC_DEBUG_MCP === '1';
function debugRuntime(...args) {
  if (DEBUG_RUNTIME) {
    const ts = new Date().toISOString();
    process.stderr.write(`[MCC-ImageRuntime ${ts}] ${args.join(' ')}\n`);
  }
}
...
function analyzeViaCliProxy(base64Data, mediaType, model, prompt, timeoutMs) {
  return new Promise((resolve, reject) => {
    ...
    debugRuntime('API call:', endpoint.toString());
    debugRuntime('Model:', model, 'Format:', format);
    debugRuntime('Prompt:', prompt.substring(0, 100));
    debugRuntime('Image size:', Math.round(base64Data.length * 3 / 4 / 1024), 'KB');
    ...
    const req = transport.request(
      ...
      (res) => {
        ...
        res.on('end', () => {
          debugRuntime('Response status:', res.statusCode, 'body length:', data.length);
          if (res.statusCode >= 400) {
            debugRuntime('Error response:', data.substring(0, 300));
          }
